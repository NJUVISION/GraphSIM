function LMN_cg=colorgradient_cg(center,pointset,pointset_d, weight, weight_d)

%      wr=G.W;
%      wd=G_d.W;
     index_center1=find(ismember(pointset(:,1:3),center(1,1:3),'rows')==1); 
     index_center2=find(ismember(pointset_d(:,1:3),center(1,1:3),'rows')==1); 
     pointset1=pointset((weight~=0),:);
     ww=weight;
     w1=ww(1,ww~=0);
     
     pointset2=pointset_d((weight_d~=0),:);
     ww=weight_d;
     w2=ww(1,ww~=0);
     N1=size(pointset1,1); 
     N2=size(pointset2,1); 
     ...
     if min_points>1
        ...
        for index_min=1:1:min_index
            ...
            for i=1:1:3
                gradient_ori1(:,i)=sqrt(rearray_w').*(centerlmn(:,i)-rearray_colorlmn(:,i));
                gradient_ori2(:,i)=sqrt(w_s').*(centerlmn(:,i)-min_matrix_colorlmn(:,i));
            end
